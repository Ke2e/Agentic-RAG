...
const directAnswerNode = async (state) => {
  ...
  const generation = await logAnswer(await model.invoke(DIRECT_PROMPT(state.question)));
  return { generation };
};
...
const generateNode = async (state) => {
  ...
  const generation = await logAnswer(await model.invoke(GEN_STRICT_PROMPT({ context, question: state.question })));
  return { generation };
};
